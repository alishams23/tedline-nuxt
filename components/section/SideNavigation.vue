<template>
  <v-navigation-drawer color="auto" class="hidden-sm-and-down border-none" location="right" rail-width="90" rail
    ...
    <div class="w-50 mt-5 mb-auto">
      ...
      <v-btn v-for="(item,index) in menu" variant="flat" :key="item + '+sidebar'"
        ...
        :class="currentRouteCheck(item.to) ? 'bg-blue-gradient text-white' : 'bg-auto  '" 
        :icon="currentRouteCheck(item.to) ? 'fa ' + item.icon : 'far ' + item.icon"></v-btn>
        <v-btn  variant="flat" :key="item + '+sidebar'"
        elevation="0" @click="changeComponentData" size="x-small" width="50" height="50" class="my-3 custom-rounded-1"
        :class="'bg-auto '"
